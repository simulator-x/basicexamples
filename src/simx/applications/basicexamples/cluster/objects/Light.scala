...
import simplex3d.math.float.functions.radians
 ...
 */
case class Light(name : String, pos : ConstVec3, rot : ConstVec3) extends EntityDescription(
    SpotLight(
      name           = name,
      transformation = ConstMat4(Mat4x3.rotateZ(radians(rot.z)) rotateY radians(rot.y) rotateX radians(rot.x) translate pos),
