...
case class Universe() extends EntityDescription(
    Subelement( Ball("sun",   0.05f,  ConstVec3f(-.7f, 1f, -7f))),
    Subelement( Ball("moon",  0.07f,  ConstVec3f(-.5f, 1f, -7f))),
    Subelement( Ball("earth", 0.1f,   ConstVec3f(-.3f, 1f, -7f)) ),
    PhysSphere(mass = 0, radius = 0.8f, transform = ConstVec3f(-.5f, -2f, -7f))
  )