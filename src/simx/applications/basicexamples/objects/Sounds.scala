...
import simx.components.sound.{SoundProps, MaterialBasedSound}
...
object Sounds {
  ...
  val ballWoodCollision = new EntityDescription(
    MaterialBasedSound(
      ...
      SValSet(types.Material("ball"), types.Material("wood")),
  ...
  val ballBallCollision = new EntityDescription(
    MaterialBasedSound(
      ...
      eventDesc = PhysicsEvents.collision,
      SValSet(types.Material("ball"), types.Material("ball")),
      soundprops = SoundProperties(gain = 1.0f)
    )
  )
}
