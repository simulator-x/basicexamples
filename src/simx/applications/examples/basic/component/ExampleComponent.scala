...
import simx.applications.examples.basic.ontology.types.{Transformation => LocalTrafo}
...
object ExampleComponent{
  val componentType = OntologySymbol('exampleComponent)
}


/**
 * The base class for all aspects for the [[simx.applications.examples.basic.component.ExampleComponent]]
 *
...
/**
 * An sphere aspect for the [[simx.applications.examples.basic.component.ExampleComponent]]
 *
 * @author Dennis Wiebusch
...
case class ExampleComponentAspect(name : Symbol, args : Any*)
  extends ComponentAspect[ExampleComponent](ExampleComponent.componentType, name, args)
{
 ...
 *
 * @author Dennis Wiebusch
 * @param name the name of this component which may be used in the target list of aspects
 */
class ExampleComponent(val name : Symbol) extends Component(name, ExampleComponent.componentType){
  ...
  private case class LocalEntityRep(id : Identifier.dataType,
                                    var posSVar : Option[StateParticle[LocalTrafo.dataType]],
  ...
  protected def entityConfigComplete(e: Entity , aspect: EntityAspect) {
    ...
    // store trafo svar
    toSimulate.posSVar  = e.getSVars(LocalTrafo).headOption.map(_._2)
    // add local rep to
