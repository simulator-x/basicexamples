...
package simx.applications.examples.basic.syncgroup
...
import simx.applications.examples.basic.objects.Ball
import simx.applications.examples.basic.objects.Light
import simx.applications.examples.basic.objects.Table
...
/**
 * This application simulated the same scene as [[simx.applications.examples.basic.ExampleApplication]] but is starting
...
class ExampleSyncGroupApplication extends SimXApplication with JVRInit with SyncGroupCreationHandling with EntityUpdateHandling{
  ...
  override protected def createEntities() {
    ...
    Execute inParallel
      Table("the table", Vec3f(3f, 1f, 2f), Vec3f(0f, -1.5f, -7f) ).realize and
      Light("the light", Vec3f(-4f, 8f, -7f), Vec3f(270f, -25f, 0f) ).realize and
      Ball ("the ball", 0.2f,  Vec3f(0f, 1.5f, -7f) ).realize exec {
      case (_, ball) =>
 ...
 */
class SyncedActor( entity : Entity, syncGroup : SyncGroup ) extends SVarActor with SyncGroupHandling with EntityUpdateHandling{

  require( entity != null, "The parameter 'entity' must not be 'null'!" )
  require( entity.getSVars( simx.core.ontology.types.Transformation ).nonEmpty,
  ...
  override def startUp() {
    syncOn( syncGroup, (x:SyncGroup) => {
      ...
      entity.getSVars(simx.core.ontology.types.Transformation).head._2.get( (t) => println( t ) )} )
    entity.getSVars( simx.core.ontology.types.Transformation ).head._2.observe(
